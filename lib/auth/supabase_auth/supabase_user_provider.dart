...
class EasyLaudosProjetoSupabaseUser extends BaseAuthUser {
  EasyLaudosProjetoSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> easyLaudosProjetoSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = EasyLaudosProjetoSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
